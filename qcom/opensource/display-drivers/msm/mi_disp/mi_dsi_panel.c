...
static void
mi_disp_set_dimming_delayed_work_handler(struct kthread_work *work) {
...
int mi_dsi_update_switch_cmd(struct dsi_panel *panel) {
  ...
  if (gamma_cfg || need_update_gamma_zero) {
    mi_dsi_panel_update_cmd_set(panel, cur_mode, DSI_CMD_SET_TIMING_SWITCH,
                                info, &gamma_cfg, sizeof(gamma_cfg));
  } else {
...
int mi_dsi_update_switch_cmd_several_gamma(struct dsi_panel *panel) {
  ...
  for (j = 0; j < cmd_update_count; j++) {
    mi_dsi_panel_update_cmd_set(panel, cur_mode, DSI_CMD_SET_TIMING_SWITCH,
                                info, &gamma_cfg[j], sizeof(gamma_cfg[j]));
    info++;
  }
  ...
  if (mi_cfg->in_fod_calibration || is_hbm_fod_on(panel)) {
    if (bl_lvl != 0) {
      if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == L2_PANEL_PA ||
          ...
          mi_get_panel_id(panel->mi_cfg.mi_panel_id) == N16_PANEL_PB ||
          mi_get_panel_id(panel->mi_cfg.mi_panel_id) == N16_PANEL_PC) {
        DISP_INFO("[%s] update 51 reg to %d even LHBM is on,", panel->type,
                  bl_lvl);
        return false;
      }
      ...
      if (feature_val == LOCAL_HBM_NORMAL_WHITE_1000NIT ||
        ...
        mi_dsi_update_lhbm_cmd_87reg(
            panel, DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_1000NIT, bl_lvl);
        ...
        mi_dsi_update_lhbm_cmd_87reg(
            panel, DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_110NIT, bl_lvl);
        dsi_panel_tx_cmd_set(panel,
...
int mi_dsi_panel_parse_cmd_sets_update(struct dsi_panel *panel,
  ...
  for (i = 0; i < DSI_CMD_UPDATE_MAX; i++) {
    arr = utils->get_property(utils->data, cmd_set_update_map[i], &length);
    ...
    rc = utils->read_u32_array(utils->data, cmd_set_update_map[i], arr_32,
                               length);
    if (rc) {
    ...
    for (k = 0; k < length; k += 3) {
      ...
      DISP_INFO("update [%s] mipi_address(0x%02X) index(%d) lenght(%d)\n",
                cmd_set_update_map[i], info->mipi_address, info->index,
                info->length);
      info++;
    }
  }
    ...
    if (tx_buf && tx_buf[0] == info->mipi_address && tx_len >= info->length) {
      ...
      if (tx_buf) {
        DISP_ERROR("[%s] %s mipi address(0x%02X != 0x%02X)\n", panel->type,
                   cmd_set_prop_map[type], tx_buf[0], info->mipi_address);
      } else {
...
int mi_dsi_panel_read_batch_number(struct dsi_panel *panel) {
  ...
  struct panel_batch_info info[] = {
      {0x00, "P0.0"}, {0x01, "P0.1"}, {0x10, "P1.0"},
      {0x11, "P1.1"}, {0x12, "P1.2"}, {0x13, "P1.2"},
      {0x20, "P2.0"}, {0x21, "P2.1"}, {0x30, "MP"},
  };
...
int mi_dsi_panel_write_dsi_cmd_set(struct dsi_panel *panel, int type) {
  ...
  for (i = 0; i < cmd_count; i++) {
    ...
    for (j = 0; j < cmds->msg.tx_len; j++) {
      buf_count += snprintf(buffer + buf_count, buf_size - buf_count, " %02X",
                            tx_buf[j]);
    }
    DISP_DEBUG("[%d] %s\n", i, buffer);
    cmds++;
...
int mi_dsi_panel_update_doze_cmd_locked(struct dsi_panel *panel, u8 value) {
  ...
  for (i = 0; i < cmd_update_count; i++) {
    DISP_INFO("[%s] update [%s] mipi_address(0x%02X) index(%d) lenght(%d)\n",
              panel->type, cmd_set_prop_map[info->type], info->mipi_address,
              info->index, info->length);
    rc = mi_dsi_panel_update_cmd_set(panel, cur_mode, DSI_CMD_SET_MI_DOZE_LBM,
                                     info, &value, sizeof(value));
    info++;
  }
...
int mi_dsi_panel_set_brightness_clone(struct dsi_panel *panel,
  ...
  mi_disp_feature_event_notify_by_type(
      disp_id, MI_DISP_EVENT_BRIGHTNESS_CLONE,
...
int mi_dsi_panel_switch_page_locked(struct dsi_panel *panel, u8 page_index) {
  ...
  for (i = 0; i < cmd_update_count; i++) {
    DISP_DEBUG("[%s] update [%s] mipi_address(0x%02X) index(%d) lenght(%d)\n",
               panel->type, cmd_set_prop_map[info->type], info->mipi_address,
               info->index, info->length);
    mi_dsi_panel_update_cmd_set(panel, cur_mode, DSI_CMD_SET_MI_SWITCH_PAGE,
                                info, tx_buf, sizeof(tx_buf));
    info++;
  }
...
int mi_dsi_panel_read_and_update_flat_param_locked(struct dsi_panel *panel) {
  ...
  panel->mipi_device.mode_flags |= MIPI_DSI_MODE_LPM;
  read_len =
      mipi_dsi_dcs_read(&panel->mipi_device, 0xB8, rdbuf, request_rx_len);
  panel->mipi_device.mode_flags = mode_flags_backup;
  ...
  for (i = 0; i < num_display_modes; i++) {
    ...
    for (j = 0; j < cmd_update_count; j++) {
      DISP_DEBUG("[%s] update [%s] mipi_address(0x%02X) index(%d) lenght(%d)\n",
                 panel->type, cmd_set_prop_map[info->type], info->mipi_address,
                 info->index, info->length);
      mi_dsi_panel_update_cmd_set(panel, cur_mode, DSI_CMD_SET_MI_FLAT_MODE_ON,
                                  info, flat_cfg->flat_on_data,
                                  sizeof(flat_cfg->flat_on_data));
      info++;
    }
  }
  ...
  for (i = 0; i < num_display_modes; i++) {
    ...
    for (j = 0; j < cmd_update_count; j++) {
      DISP_DEBUG("[%s] update [%s] mipi_address(0x%02X) index(%d) lenght(%d)\n",
                 panel->type, cmd_set_prop_map[info->type], info->mipi_address,
                 info->index, info->length);
      mi_dsi_panel_update_cmd_set(panel, cur_mode, DSI_CMD_SET_MI_FLAT_MODE_OFF,
                                  info, flat_cfg->flat_off_data,
                                  sizeof(flat_cfg->flat_off_data));
      info++;
    }
  }
...
int mi_dsi_panel_read_and_update_dc_param_locked(struct dsi_panel *panel) {
  ...
  for (read_count = DC_LUT_60HZ; read_count < DC_LUT_MAX; read_count++) {
    ...
    DISP_INFO("[%s]read DC 0x%02X paramter length is %d\n", panel->type,
              read_dc_reg, request_rx_len);

    mode_flags_backup = panel->mipi_device.mode_flags;
    panel->mipi_device.mode_flags |= MIPI_DSI_MODE_LPM;
    read_len = mipi_dsi_dcs_read(&panel->mipi_device, read_dc_reg, read_buf,
                                 request_rx_len);
    panel->mipi_device.mode_flags = mode_flags_backup;
    if (read_len == request_rx_len) {
      ...
      for (i = 0; i < sizeof(dc_cfg->exit_dc_lut[read_count]); i++) {
        DISP_DEBUG("DC 0x%02X exit_dc_lut[%d] = 0x%02X\n", read_dc_reg, i,
                   dc_cfg->exit_dc_lut[read_count][i]);
      }
    } else {
    ...
    for (i = 0; i < sizeof(dc_cfg->enter_dc_lut[read_count]); i++) {
      DISP_DEBUG("DC 0x%02X enter_dc_lut[%d] = 0x%02X\n", read_dc_reg, i,
                 dc_cfg->enter_dc_lut[read_count][i]);
    }
  ...
  for (i = 0; i < num_display_modes; i++) {
    ...
    for (j = 0; j < cmd_update_count; j++) {
      ...
      mi_dsi_panel_update_cmd_set(panel, cur_mode, DSI_CMD_SET_MI_DC_ON, info,
                                  dc_cfg->enter_dc_lut[j],
                                  sizeof(dc_cfg->enter_dc_lut[j]));
      info++;
    }
    ...
    for (j = 0; j < cmd_update_count; j++) {
      DISP_INFO("[%s] update [%s] mipi_address(0x%02X) index(%d) lenght(%d)\n",
                panel->type, cmd_set_prop_map[info->type], info->mipi_address,
                info->index, info->length);
      mi_dsi_panel_update_cmd_set(panel, cur_mode, DSI_CMD_SET_MI_DC_OFF, info,
                                  dc_cfg->exit_dc_lut[j],
                                  sizeof(dc_cfg->exit_dc_lut[j]));
      info++;
    }
  }
...
int mi_dsi_update_51_mipi_cmd(struct dsi_panel *panel,
  ...
  for (j = 0; j < cmd_update_count; j++) {
    ...
    if (info && info->mipi_address != 0x51) {
      ...
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, bl_buf,
                                  sizeof(bl_buf));
      break;
    }
  }
...
static int mi_dsi_update_lhbm_cmd_87reg(struct dsi_panel *panel,
  ...
  for (j = 0; j < cmd_update_count; j++) {
    ...
    if (info && info->mipi_address != 0x87) {
      ...
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, alpha_buf,
                                  info->length);
      break;
    }
  }
...
static int mi_dsi_update_lhbm_cmd_N16_PC(struct dsi_panel *panel,
                                         enum dsi_cmd_set_type type,
                                         int bl_lvl) {
  struct dsi_display_mode_priv_info *priv_info = NULL;
  struct dsi_cmd_update_info *info = NULL;
...
static int mi_dsi_update_lhbm_cmd_DF_reg(struct dsi_panel *panel,
  ...
  for (j = 0; j < cmd_update_count; j++) {
    ...
    if (info && info->mipi_address != 0xDF) {
      ...
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, df_reg_buf,
                                  sizeof(df_reg_buf));
      break;
    }
  }
...
static int mi_dsi_update_lhbm_cmd_63_C5_reg(struct dsi_panel *panel,
  ...
  DISP_DEBUG("[%s] bl_lvl = %d, 63 reg = 0x%02X 0x%02X 0x%02X 0x%02X\n",
             panel->type, bl_lvl, reg_63_buf[0], reg_63_buf[1], reg_63_buf[2],
             reg_63_buf[3]);
  ...
  memcpy(&reg_C5_buf[0], &panel->mi_cfg.lhbm_rgb_param[6 * (rgb_gamma_num - 1)],
         sizeof(u8) * 6);

  memcpy(&reg_C5_buf[6], &panel->mi_cfg.lhbm_rgb_param[6 * (rgb_gamma_num - 1)],
         sizeof(u8) * 6);

  memcpy(&reg_C5_buf[12],
         &panel->mi_cfg.lhbm_rgb_param[6 * (rgb_gamma_num - 1)],
         sizeof(u8) * 6);

  DISP_DEBUG("[%s] C5 reg = 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X\n",
             panel->type, reg_C5_buf[0], reg_C5_buf[1], reg_C5_buf[2],
             reg_C5_buf[3], reg_C5_buf[4], reg_C5_buf[5]);
  ...
  for (i = 0; i < cmd_update_count; i++) {
    if (info) {
      ...
      if (info->mipi_address == 0x63) {
        mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, reg_63_buf,
                                    sizeof(reg_63_buf));
      } else if (info->mipi_address == 0xC5) {
        mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, reg_C5_buf,
                                    sizeof(reg_C5_buf));
      } else if (info->mipi_address == 0x51) {
        mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, reg_51_buf,
                                    sizeof(reg_51_buf));
      }
      info++;
    }
...
static int mi_dsi_update_lhbm_cmd_B2_reg(struct dsi_panel *panel,
  ...
  for (j = 0; j < cmd_update_count; j++) {
    ...
    if (info && info->mipi_address != 0xB2) {
      ...
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, b2_reg_buf,
                                  sizeof(b2_reg_buf));
      break;
    }
  }
...
int mi_dsi_update_nolp_cmd_B2reg(struct dsi_panel *panel,
  ...
  mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, set_buf,
                              sizeof(set_buf));
...
int mi_dsi_panel_read_manufacturer_info(struct dsi_panel *panel,
                                        u32 manufacturer_info_addr, char *rdbuf,
                                        int rdlen) {
  int ret = 0;
  ...
  mutex_lock(&panel->panel_lock);
  ret = mi_dsi_panel_read_manufacturer_info_locked(
      panel, manufacturer_info_addr, rdbuf, rdlen);
  mutex_unlock(&panel->panel_lock);
  return ret;
}
int mi_dsi_panel_read_manufacturer_info_locked(struct dsi_panel *panel,
                                               u32 manufacturer_info_addr,
                                               char *rdbuf, int rdlen) {
  int rc = 0;
  ...
  if (mi_get_panel_id_by_dsi_panel(panel) == L18_PANEL_PA) {
    rc = mipi_dsi_dcs_write(&panel->mipi_device, cmd, tesk_key_enable,
                            sizeof(tesk_key_enable));
    if (rc < 0) {
      DISP_ERROR("[%s] wrie tesk_key_enable failed, rc=%d\n", panel->type, rc);
      goto exit;
    }
  }
  rc = mipi_dsi_dcs_read(&panel->mipi_device, manufacturer_info_addr, rdbuf,
                         rdlen);
  if (rc < 0) {
  ...
  if (mi_get_panel_id_by_dsi_panel(panel) == L18_PANEL_PA) {
    rc = mipi_dsi_dcs_write(&panel->mipi_device, cmd, tesk_key_disable,
                            sizeof(tesk_key_disable));
    if (rc < 0) {
      DISP_ERROR("[%s] wrie tesk_key_enable failed, rc=%d\n", panel->type, rc);
      goto exit;
    }
...
int mi_dsi_panel_read_lhbm_white_param_locked(struct dsi_panel *panel) {
  ...
  if (mi_cfg->lhbm_w1000_update_flag) {
    ...
    rc = dsi_panel_tx_cmd_set(
        panel, DSI_CMD_SET_MI_LOCAL_HBM_WHITE_1000NIT_GIRON_PRE_READ);
    if (rc) {
      DISP_ERROR(
          "[%s] failed to send HBM_WHITE_1000NIT_GIRON_PRE_READ cmds, rc=%d\n",
          panel->type, rc);
      rc = -EINVAL;
      goto exit;
    }
    ...
    rc = dsi_panel_tx_cmd_set(
        panel, DSI_CMD_SET_MI_LOCAL_HBM_WHITE_1000NIT_GIROFF_PRE_READ);
    if (rc) {
      DISP_ERROR(
          "[%s] failed to send HBM_WHITE_1000NIT_GIROFF_PRE_READ cmds, rc=%d\n",
          panel->type, rc);
      rc = -EINVAL;
      goto exit;
    }
    ...
    for (j = 0; j < sizeof(mi_cfg->whitebuf_1000_gir_off); j++)
      DISP_INFO(
          "read from 0xB2,0xB5, 0xB8,whitebuf_1000_gir_off[%d] = 0x%02X\n", j,
          mi_cfg->whitebuf_1000_gir_off[j]);
  } else {
    mi_cfg->lhbm_w1000_readbackdone = true;
  }
  ...
  if (mi_cfg->lhbm_w110_update_flag) {
    ...
    rc = dsi_panel_tx_cmd_set(
        panel, DSI_CMD_SET_MI_LOCAL_HBM_WHITE_110NIT_GIRON_PRE_READ);
    if (rc) {
      DISP_ERROR(
          "[%s] failed to send HBM_WHITE_110NIT_GIRON_PRE_READ cmds, rc=%d\n",
          panel->type, rc);
      rc = -EINVAL;
      goto exit;
    }
    ...
    rc = dsi_panel_tx_cmd_set(
        panel, DSI_CMD_SET_MI_LOCAL_HBM_WHITE_110NIT_GIROFF_PRE_READ);
    if (rc) {
      DISP_ERROR(
          "[%s] failed to send HBM_WHITE_110NIT_GIROFF_PRE_READ cmds, rc=%d\n",
          panel->type, rc);
      rc = -EINVAL;
      goto exit;
    }
...
static int mi_dsi_panel_update_lhbm_white_param(struct dsi_panel *panel,
  ...
  if (!panel->mi_cfg.lhbm_w1000_update_flag &&
      !panel->mi_cfg.lhbm_w110_update_flag &&
      !panel->mi_cfg.lhbm_g500_update_flag) {
  ...
  if (panel->mi_cfg.uefi_read_lhbm_success &&
      (!panel->mi_cfg.lhbm_w1000_readbackdone ||
       !panel->mi_cfg.lhbm_w110_readbackdone)) {
    for (i = 0; i < sizeof(panel->mi_cfg.lhbm_rgb_param); i++) {
      if (i < 6)
        panel->mi_cfg.whitebuf_1000_gir_on[i] = panel->mi_cfg.lhbm_rgb_param[i];
      else if (i < 12)
        panel->mi_cfg.whitebuf_1000_gir_off[i - 6] =
            panel->mi_cfg.lhbm_rgb_param[i];
      else if (i < 18)
        panel->mi_cfg.whitebuf_110_gir_on[i - 12] =
            panel->mi_cfg.lhbm_rgb_param[i];
      else if (i < 24)
        panel->mi_cfg.whitebuf_110_gir_off[i - 18] =
            panel->mi_cfg.lhbm_rgb_param[i];
    }
    ...
    if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == L9S_PANEL_PA) {
      for (i = 0; i < sizeof(panel->mi_cfg.lhbm_rgb_param); i++) {
        if (i < 6) {
          panel->mi_cfg.whitebuf_1000_gir_on[i] =
              panel->mi_cfg.lhbm_rgb_param[i];
          panel->mi_cfg.whitebuf_1000_gir_off[i] =
              panel->mi_cfg.lhbm_rgb_param[i];
        } else if (i < 12) {
          panel->mi_cfg.whitebuf_110_gir_off[i - 6] =
              panel->mi_cfg.lhbm_rgb_param[i];
        } else if (i < 18) {
          panel->mi_cfg.whitebuf_110_gir_on[i - 12] =
              panel->mi_cfg.lhbm_rgb_param[i];
        } else if (i < 24) {
          panel->mi_cfg.greenbuf_500nit[i - 18] =
              panel->mi_cfg.lhbm_rgb_param[i];
        }
      }
      for (i = 2; i < 4; i++) {
    ...
    if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == L9S_PANEL_PB) {
      for (i = 0; i < 18; i++) {
        if (i < 6) {
          ...
          panel->mi_cfg.whitebuf_110_gir_on[i - 6] =
              panel->mi_cfg.lhbm_rgb_param[i];
          panel->mi_cfg.whitebuf_110_gir_off[i - 6] =
              panel->mi_cfg.lhbm_rgb_param[i];
        } else if (i < 18) {
          panel->mi_cfg.greenbuf_500nit[i - 12] =
              panel->mi_cfg.lhbm_rgb_param[i];
        }
      }
      for (i = 2; i < 4; i++) {
    ...
    if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == L12_PANEL_PB) {
      /*  900nit GAMMA R=R[11:0]*1.09; 900nit GAMMA G=G[11:0]*1.08; 900nit GAMMA
         B=B[11:0]*1.09 110nit GAMMA R=R[11:0]*1.03; 110nit GAMMA
         G=G[11:0]*1.03; 110nit GAMMA B=B[11:0]*1.03
      */
      for (i = 0; i < 4; i++) {
        if (i == 0) {
          coefficient = 109;
    ...
    if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == M11A_PANEL_PA) {
      /*  250nit GAMMA R=R[11:0]+70; 250nit GAMMA G=G[11:0]+70; 250nit GAMMA
       * B=B[11:0]+70
       */
      for (i = 0; i < 2; i++) {
        if (i == 0)
      ...
      if (level == 110) {
        if (flat_mode != 0) {
          mi_dsi_panel_update_cmd_set(
              panel, cur_mode, type, info, panel->mi_cfg.whitebuf_110_gir_on,
              sizeof(panel->mi_cfg.whitebuf_110_gir_on));
        } else {
          mi_dsi_panel_update_cmd_set(
              panel, cur_mode, type, info, panel->mi_cfg.whitebuf_110_gir_off,
              sizeof(panel->mi_cfg.whitebuf_110_gir_off));
        }
      } else if (level == 500) {
        mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info,
                                    panel->mi_cfg.greenbuf_500nit,
                                    sizeof(panel->mi_cfg.greenbuf_500nit));
      } else {
        if (flat_mode != 0) {
          mi_dsi_panel_update_cmd_set(
              panel, cur_mode, type, info, panel->mi_cfg.whitebuf_1000_gir_on,
              sizeof(panel->mi_cfg.whitebuf_1000_gir_on));
        } else {
          mi_dsi_panel_update_cmd_set(
              panel, cur_mode, type, info, panel->mi_cfg.whitebuf_1000_gir_off,
              sizeof(panel->mi_cfg.whitebuf_1000_gir_off));
        }
      }
    }
  }
...
static int mi_dsi_panel_set_lhbm_fod_locked(struct dsi_panel *panel,
  ...
  case LOCAL_HBM_NORMAL_WHITE_1000NIT:
    ...
    mi_dsi_update_51_mipi_cmd(
        panel, DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_1000NIT, update_bl);

    mi_dsi_update_lhbm_cmd_87reg(
        panel, DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_1000NIT, update_bl);

    mi_dsi_update_lhbm_cmd_63_C5_reg(
        panel, DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_1000NIT, update_bl);

    if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == N16_PANEL_PB)
      mi_dsi_update_lhbm_cmd_N16_PB(
          panel, DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_1000NIT, update_bl);

    if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == N16_PANEL_PC)
      mi_dsi_update_lhbm_cmd_N16_PC(
          panel, DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_1000NIT, update_bl);

    mi_dsi_update_lhbm_cmd_DF_reg(
        panel, DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_1000NIT, update_bl);

    mi_dsi_update_lhbm_cmd_B2_reg(
        panel, DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_1000NIT, update_bl);

    mi_dsi_panel_update_lhbm_white_param(
        panel, DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_1000NIT,
  ...
  case LOCAL_HBM_NORMAL_WHITE_110NIT:
    ...
    if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == N16_PANEL_PB)
      mi_dsi_update_lhbm_cmd_N16_PB(
          panel, DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_110NIT, update_bl);
    if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == N16_PANEL_PC)
      mi_dsi_update_lhbm_cmd_N16_PC(
          panel, DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_110NIT, update_bl);
    mi_dsi_update_lhbm_cmd_63_C5_reg(
    ...
    mi_dsi_update_lhbm_cmd_B2_reg(
        panel, DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_110NIT, update_bl);
    mi_dsi_panel_update_lhbm_white_param(
        panel, DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_110NIT,
        mi_cfg->feature_val[DISP_FEATURE_FLAT_MODE]);
    rc = dsi_panel_tx_cmd_set(panel,
                              DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_110NIT);
    break;
  case LOCAL_HBM_NORMAL_GREEN_500NIT:
    ...
    if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == N16_PANEL_PB)
      mi_dsi_update_lhbm_cmd_N16_PB(
          panel, DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_GREEN_500NIT,
          mi_cfg->last_bl_level);
    if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == N16_PANEL_PC)
      mi_dsi_update_lhbm_cmd_N16_PC(
          panel, DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_GREEN_500NIT,
  ...
  case LOCAL_HBM_HLPM_WHITE_1000NIT:
    ...
    mi_cfg->dimming_state = STATE_DIM_BLOCK;
    update_bl = mi_cfg->last_bl_level;
    mi_dsi_update_nolp_cmd_B2reg(panel,
    ...
    mi_dsi_update_51_mipi_cmd(
        panel, DSI_CMD_SET_MI_LOCAL_HBM_HLPM_WHITE_1000NIT, update_bl);
    mi_dsi_update_lhbm_cmd_87reg(
        panel, DSI_CMD_SET_MI_LOCAL_HBM_HLPM_WHITE_1000NIT, update_bl);
    mi_dsi_update_lhbm_cmd_B2_reg(
        panel, DSI_CMD_SET_MI_LOCAL_HBM_HLPM_WHITE_1000NIT, update_bl);
    ...
    mi_dsi_panel_update_lhbm_white_param(
        panel, DSI_CMD_SET_MI_LOCAL_HBM_HLPM_WHITE_1000NIT,
        mi_cfg->feature_val[DISP_FEATURE_FLAT_MODE]);
  ...
  case LOCAL_HBM_HLPM_WHITE_110NIT:
    ...
    mi_dsi_update_51_mipi_cmd(panel, DSI_CMD_SET_MI_LOCAL_HBM_HLPM_WHITE_110NIT,
                              update_bl);
    mi_dsi_update_lhbm_cmd_87reg(
        panel, DSI_CMD_SET_MI_LOCAL_HBM_HLPM_WHITE_110NIT, update_bl);
    mi_dsi_update_lhbm_cmd_B2_reg(
        panel, DSI_CMD_SET_MI_LOCAL_HBM_HLPM_WHITE_110NIT, update_bl);
    mi_dsi_update_lhbm_cmd_63_C5_reg(
        panel, DSI_CMD_SET_MI_LOCAL_HBM_HLPM_WHITE_110NIT, update_bl);
    ...
    mi_dsi_update_backlight_in_aod(panel, false);
    mi_dsi_panel_update_lhbm_white_param(
        panel, DSI_CMD_SET_MI_LOCAL_HBM_HLPM_WHITE_110NIT,
        mi_cfg->feature_val[DISP_FEATURE_FLAT_MODE]);
    rc =
...
static int mi_dsi_panel_gray_by_temperature(struct dsi_panel *panel,
                                            int temp_level) {
  int rc = 0;
  struct dsi_cmd_update_info *info = NULL;
  struct dsi_display_mode *cur_mode = NULL;
  ...
  mi_dsi_panel_update_cmd_set(panel, cur_mode, DSI_CMD_SET_MI_3D_LUT_MODE, info,
...
int mi_dsi_panel_set_disp_param(struct dsi_panel *panel,
...
#ifdef DISPLAY_FACTORY_BUILD
    if (dsi_panel_initialized(panel) && is_aod_brightness(ctl->feature_val)) {
  ...
  case DISP_FEATURE_FLAT_MODE:
    ...
    if (!mi_cfg->flat_sync_te) {
      ...
      mi_disp_feature_event_notify_by_type(
          mi_get_disp_id(panel->type), MI_DISP_EVENT_FLAT_MODE,
          sizeof(ctl->feature_val), ctl->feature_val);
    } else {
  ...
  case DISP_FEATURE_DC:
    ...
    if (mi_cfg->dc_feature_enable) {
      ...
      rc = mi_dsi_panel_set_dc_mode_locked(panel,
                                           ctl->feature_val == FEATURE_ON);
    }
    mi_cfg->feature_val[DISP_FEATURE_DC] = ctl->feature_val;
    mi_disp_feature_event_notify_by_type(
        mi_get_disp_id(panel->type), MI_DISP_EVENT_DC, sizeof(ctl->feature_val),
        ctl->feature_val);
    break;
  ...
  case DISP_FEATURE_FP_STATUS:
    ...
    if (ctl->feature_val == ENROLL_STOP || ctl->feature_val == AUTH_STOP ||
  ...
  case DISP_FEATURE_AOD_TO_NORMAL:
    rc = mi_dsi_panel_aod_to_normal_optimize_locked(panel, ctl->feature_val ==
                                                               FEATURE_ON);
  ...
  case DISP_FEATURE_DBI:
    if ((mi_get_panel_id(panel->mi_cfg.mi_panel_id) == N16_PANEL_PB) || 
       (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == N16_PANEL_PC)) {
      if (panel->mi_cfg.id_config.build_id != 0x00 &&
          panel->mi_cfg.id_config.build_id != 0x10) {
  ...
  case DISP_FEATURE_DDIC_ROUND_CORNER:
    ...
    rc = mi_dsi_panel_set_round_corner_locked(panel,
                                              ctl->feature_val == FEATURE_ON);
...
ssize_t mi_dsi_panel_show_disp_param(struct dsi_panel *panel, char *buf,
  ...
  count =
      snprintf(buf, size, "%040s: feature vaule\n", "feature name[feature id]");
...
int mi_dsi_panel_parse_flatmode_configs(struct dsi_panel *panel) {
  ...
  data = utils->find_property(
      utils->data, "mi,mdss-dsi-panel-flatmode-on-status-value", &tmp);
  tmp /= sizeof(u32);
...
int mi_dsi_pwr_enable_vregs(struct dsi_regulator_info *regs, bool enable,
  ...
  if (enable) {
    ...
    if (num_of_v > 0) {
      rc = regulator_set_voltage(vreg->vreg, vreg->min_voltage,
                                 vreg->max_voltage);
      if (rc) {
...
int dsi_panel_parse_wp_reg_read_config(struct dsi_panel *panel) {
  ...
  dsi_panel_parse_cmd_sets_sub(&wp_config->wp_cmd, DSI_CMD_SET_PANEL_WP_READ,
                               utils);
  ...
  wp_config->return_buf =
      kcalloc(wp_config->wp_cmds_rlen, sizeof(unsigned char), GFP_KERNEL);
